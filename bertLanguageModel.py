...
## Required parameters
flags.DEFINE_string(
    "data_dir", "/home/wxl/bertProject/bertTextClassification/data/balanceData",
...
flags.DEFINE_string("task_name", "multiclass", "The name of the task to train.")

flags.DEFINE_string("vocab_file", "/home/wxl/bertProject/chinese_L-12_H-768_A-12/vocab.txt",
...
class MultiClassProcessor(DataProcessor):
  ...
  def get_labels(self):
    """See base class."""
    with open(filePathLabel, encoding='utf-8') as fr:
    #with open("/home/wxl/bertProject/bertTextClassification/data/bigData/filteredTop3000LabelsV1.tsv") as fr:
...
def isType(mystr):
    for item in mystr:
        if (item>='a'and item<='z') or (item >='A' and item<='Z') or item=='.':
            continue
        else:
            return False
    return True
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
  ...
  tokens_tmp = example.text_a.split(' ')
  tokens_a=[]
  for item in tokens_tmp:
      if item=='<s>':
          continue
      if isType(item):
          tokens_a.append(item)
      else:
          itemTokens=tokenizer.tokenize(item)
          tokens_a.extend(itemTokens)
  ...
  input_idsPre = tokenizer.convert_tokens_to_ids(tokens)
  input_ids=input_idsPre[:-1]
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
  ...
  # instead.
  outputSeq=model.get_sequence_output()
  nsteps=tf.shape(outputSeq)[1]
  #output_layer = model.get_pooled_output()

  #hidden_size = output_layer.shape[-1].value
  hidden_size = outputSeq.shape[-1].value
  ...
  with tf.variable_scope("loss"):
    if is_training:
      ...
      outputSeq = tf.nn.dropout(outputSeq, keep_prob=0.9)

    #logits = tf.matmul(output_layer, output_weights, transpose_b=True)
    #logits = tf.nn.bias_add(logits, output_bias)

    output = tf.reshape(outputSeq, [-1, hidden_size])
    pred = tf.matmul(output, output_weights,transpose_b=True) + output_bias
    logits = tf.reshape(pred, [-1, nsteps, num_labels])
    ...
    '''
    with open(fileWeights,encoding='utf-8') as fr:
        for line in fr:
            weights=line.strip().split('\t')
        for i in range(len(weights)):
            weights[i]=float(weights[i])

    weights = [weights for i in range(log_probs.shape[0].value)]
